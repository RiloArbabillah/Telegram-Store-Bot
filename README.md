...
Telegram bot for selling digital products: · sell software license keys on Telegram · Telegram shop/store bot · QRIS payment bot · direct checkout · automated digital delivery · Python e-commerce bot · python-telegram-bot store · SQLAlchemy SQLite Telegram bot · self-hosted digital goods storefront.
> 
...
Customers browse a catalog, checkout directly, pay by QRIS, and receive products after payment confirmation.
...
13. [Optional — DANA QRIS](#optional--dana-qris)
...
- 📱 Direct QRIS checkout for product purchases
- 🔒 Pending checkout reserves stock until payment completes, expires, or is cancelled
...
| `CRYPTO_BOT_API_KEY` | ➖ | Legacy/unused in QRIS-only direct checkout. |
| `TELEGRAM_PROVIDER_TOKEN` | ➖ | Legacy/unused in QRIS-only direct checkout. |
| `PAYMENT_CURRENCY` | ➖ | Business currency for the whole app (default `IDR`). Product, order, and payment amounts are treated as whole rupiah. |
...
2. Send **`/start`** — you’ll get the welcome message and the main menu (Products, Order History, Availability, Support).
...
**🎉 That’s it — your bot is live.** A typical first run as admin: open `/admin` → **Product Management** → create a category, then a product, then **Restock Keys** to add inventory. As a user, `/start` → browse products → checkout → pay by QRIS.
...
5. Restart the bot. When DANA config is complete, the QRIS checkout path switches from manual mode to DANA mode automatically.
...
Customers checkout a product directly and pay by **QRIS**. Manual QRIS uses proof upload and admin approval; DANA QRIS can confirm automatically by callback.
...
| QRIS not configured | Upload/set QRIS settings in admin, or configure DANA QRIS credentials. |
| QRIS payment not auto-confirming | Check DANA/payment.deka webhook configuration, or use manual proof approval. |
